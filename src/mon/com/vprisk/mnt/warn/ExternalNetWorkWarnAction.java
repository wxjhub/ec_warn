...
import com.vprisk.mnt.action.ExternalNetWorkAction;
import com.vprisk.mnt.action.WarnIntervalAction;
import com.vprisk.mnt.entity.WarnInterval;
import com.vprisk.rmplatform.util.StringUtil;

public class ExternalNetWorkWarnAction extends TimerTask{
	...
	private  synchronized void apply(){
		if(!isrunning){
		   ...
		   //根据预警代码查询关联的预警时间间隔对象
		   WarnInterval wi = wia.WarnIntervalByWarnCode("EXTERNAL_NET_WARN_INTEVAL","find",null);
			  if(StringUtil.isNotNullOrEmpty(wi.getOnceTime())){//判断上次预警时间是否为空
				...
				if(longnum>=intertime){
					//调用应用程序预警的方法
					ExternalNetWorkAction app = new ExternalNetWorkAction();
					//app.serviceServerAuto();
					app.autoControl();
					//更新预警时间
					wia.WarnIntervalByWarnCode("EXTERNAL_NET_WARN_INTEVAL","update",datedata);
				}
			}else{
				ExternalNetWorkAction app = new ExternalNetWorkAction();
				//app.serviceServerAuto();
