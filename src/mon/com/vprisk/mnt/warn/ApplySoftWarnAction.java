...
import com.vprisk.mnt.action.ApplyProSoftAction;
import com.vprisk.mnt.action.WarnIntervalAction;
import com.vprisk.mnt.entity.WarnInterval;
import com.vprisk.rmplatform.util.StringUtil;

public class ApplySoftWarnAction extends TimerTask{
	...
	private  synchronized void apply(){
		if(!isrunning){
		   ...
		   //根据预警代码查询关联的预警时间间隔对象
		   WarnInterval wi = wia.WarnIntervalByWarnCode("APPLY_SOFT_INTERVAL","find",null);
			  if(wi.getOnceTime()!=null&&StringUtil.isNotNullOrEmpty(wi.getOnceTime())){//判断上次预警时间是否为空
				...
				if(longnum>=intertime){
					//调用应用程序预警的方法
					ApplyProSoftAction app = new ApplyProSoftAction();
					//app.serviceServerAuto();
					app.serviceServer();
					//更新预警时间
					wia.WarnIntervalByWarnCode("APPLY_SOFT_INTERVAL","update",datedata);
				}
			}else{
				ApplyProSoftAction app = new ApplyProSoftAction();
				//app.serviceServerAuto();
