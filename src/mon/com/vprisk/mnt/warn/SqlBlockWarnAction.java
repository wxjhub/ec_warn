...
public class SqlBlockWarnAction extends TimerTask{
	...
	private  synchronized void apply(){
		if(!isrunning){
		   ...
		   //根据预警代码查询关联的预警时间间隔对象
		   WarnInterval wi = wia.WarnIntervalByWarnCode("SQL_BLOCK_WARN_INTEVAL","find",null);
			  if(StringUtil.isNotNullOrEmpty(wi.getOnceTime())){//判断上次预警时间是否为空
				...
				if(longnum>=intertime){
					//调用应用程序预警的方法
					SqlBlockAction app = new SqlBlockAction();
					//app.serviceServerAuto();
					app.autoControl();
					//更新预警时间
					wia.WarnIntervalByWarnCode("SQL_BLOCK_WARN_INTEVAL","update",datedata);
				}
			}else{
				SqlBlockAction app = new SqlBlockAction();
				//app.serviceServerAuto();
