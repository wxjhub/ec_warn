...
import com.vprisk.mnt.action.BtnTranStranSucssAction;
import com.vprisk.mnt.action.WarnIntervalAction;
import com.vprisk.mnt.entity.WarnInterval;
import com.vprisk.rmplatform.util.StringUtil;

public class BtnTranStranSucessAction extends TimerTask{
	...
	private  synchronized void apply(){
		if(!isrunning){
		   ...
		   //根据预警代码查询关联的预警时间间隔对象
		   WarnInterval wi = wia.WarnIntervalByWarnCode("TRANSACTION_SUCCESS_INTERVAL","find",null);
			  if(StringUtil.isNotNullOrEmpty(wi.getOnceTime())){//判断上次预警时间是否为空
				...
				if(longnum>=intertime){
					//调用应用程序预警的方法
					BtnTranStranSucssAction app = new BtnTranStranSucssAction();
					//app.serviceServerAuto();
					app.btnTranStranSucss();
					//更新预警时间
					wia.WarnIntervalByWarnCode("TRANSACTION_SUCCESS_INTERVAL","update",datedata);
				}
			}else{
				BtnTranStranSucssAction app = new BtnTranStranSucssAction();
				//app.serviceServerAuto();
