 ...
 * 
 */
public class TableAnalysisConfigAction extends BaseAction {
	...
	public String findList() {
		...
		// 调用按照分页查询信息的方法
		Page page = this.tableAnalysisConfigService.selectParameterCollectiondByPage(params, pageNo, pageSize,
						orderBy, isAsc);
	 ...
	 */
	public String save() {
		TableAnalysisConfig detail = (TableAnalysisConfig) BeanUtil.getObject(
				getHttpRequest(), TableAnalysisConfig.class);
		// 通过request方法获取页面传递的参数
		try {
			PrintWriter pw = super.getHttpResponse().getWriter();
			// 调用根据对象参数信息，对于信息新增修改时是否存在重复数据
			List<TableAnalysisConfig> list = tableAnalysisConfigService.findParameterCollection(detail);
			if (list.size() < 1) {// 判断是修改
				boolean flag = tableAnalysisConfigService.saveOrUpdateCash(detail);// 保存，调用修改的方法
				if (flag) {
					pw.print(1);
				} else {
					pw.print(3);
	...
	public String remove() {
		...
		try {
			PrintWriter pw = super.getHttpResponse().getWriter();
			tableAnalysisConfigService.removeParameterCollectionByuuid(uuid);
			pw.flush();
			pw.close();
		} catch (IOException e) {
			e.printStackTrace();
