 ...
 * 
 */
public class DayEndConfigAction extends BaseAction {
	...
	public String findList() {
		...
		// 调用按照分页查询信息的方法
		Page page = this.dayEndConfigService.selectParameterCollectiondByPage(
	 ...
	 */
	public String save() {
		DayEndConfig detail = (DayEndConfig) BeanUtil.getObject(
				getHttpRequest(), DayEndConfig.class);
		// 通过request方法获取页面传递的参数
		try {
			PrintWriter pw = super.getHttpResponse().getWriter();
			// 调用根据对象参数信息，对于信息新增修改时是否存在重复数据
			List<DayEndConfig> list = dayEndConfigService
					.findParameterCollection(detail);
			if (list.size() < 1) {// 判断是修改
				boolean flag = dayEndConfigService.saveOrUpdateCash(detail);// 保存，调用修改的方法
				if (flag) {
					pw.print(1);
				} else {
					pw.print(3);
	...
	public String remove() {
		...
		try {
			PrintWriter pw = super.getHttpResponse().getWriter();
			dayEndConfigService.removeParameterCollectionByuuid(uuid);
			pw.flush();
			pw.close();
		} catch (IOException e) {
			e.printStackTrace();
