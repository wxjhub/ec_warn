 ...
 * @author
 */
public class TransactionSuccessConfigAction extends BaseAction {

	private static final long serialVersionUID = 1L;
	private TransactionSuccessConfigService transactionSuccessConfigService = (TransactionSuccessConfigService) super
			.getBean("transactionSuccessConfigService");
	...
	public String findList() {
		...
		// 调用按照分页查询信息的方法
		Page page = this.transactionSuccessConfigService
				.selectParameterCollectiondByPage(params, pageNo, pageSize,
						orderBy, isAsc);
	 ...
	 */
	public String save() {
		TransactionSuccessConfig detail = (TransactionSuccessConfig) BeanUtil
				.getObject(getHttpRequest(), TransactionSuccessConfig.class);
		// 通过request方法获取页面传递的参数
		try {
			PrintWriter pw = super.getHttpResponse().getWriter();
			// 调用根据对象参数信息，对于信息新增修改时是否存在重复数据
			List<TransactionSuccessConfig> list = transactionSuccessConfigService
					.findParameterCollection(detail);
			if (list.size() < 1) {// 判断是修改
				boolean flag = transactionSuccessConfigService
						.saveOrUpdateCash(detail);// 保存，调用修改的方法
				if (flag) {
					pw.print(1);
				} else {
					pw.print(3);
	...
	public String remove() {
		...
		try {
			PrintWriter pw = super.getHttpResponse().getWriter();
			transactionSuccessConfigService
					.removeParameterCollectionByuuid(uuid);
			pw.flush();
			pw.close();
		} catch (IOException e) {
			e.printStackTrace();
