...
public class BtnSuccessConfigAction extends BaseAction{
	...
	public String findBtnSuccessConfigList() {
		...
		// 调用按照分页查询信息的方法
		Page page = this.btnSucessConfigService.selectParameterCollectiondByPage(
	 ...
	 */
	public String save() {
		BtnSuccessConfig detail = (BtnSuccessConfig) BeanUtil.getObject(getHttpRequest(), BtnSuccessConfig.class);
		// 通过request方法获取页面传递的参数
		try {
			PrintWriter pw = super.getHttpResponse().getWriter();
			// 调用根据对象参数信息，对于信息新增修改时是否存在重复数据
			List<BtnSuccessConfig> list = btnSucessConfigService.findParameterCollection(detail);
			if (list.size() < 1) {// 判断是修改
				boolean flag = btnSucessConfigService.saveOrUpdateCash(detail);// 保存，调用修改的方法
				if (flag) {
					pw.print(1);
				} else {
					pw.print(3);
	...
	public String remove() {
		...
		try {
			PrintWriter pw = super.getHttpResponse().getWriter();
			btnSucessConfigService.removeParameterCollectionByuuid(uuid);
			pw.flush();
			pw.close();
		} catch (IOException e) {
			e.printStackTrace();
