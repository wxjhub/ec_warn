 ...
 * @author
 */
public class TransactionTimeConfigAction extends BaseAction {

	private static final long serialVersionUID = 1L;
	private TransactionTimeConfigService transactionTimeConfigService = (TransactionTimeConfigService) super
			.getBean("transactionTimeConfigService");

	@SuppressWarnings("unchecked")
	public String findTransactionTimeList() {
		...
		// 调用按照分页查询信息的方法
		Page page = this.transactionTimeConfigService
	 ...
	 */
	public String save() {
		TransactionTimeConfig detail = (TransactionTimeConfig) BeanUtil
				.getObject(getHttpRequest(), TransactionTimeConfig.class);
		// 通过request方法获取页面传递的参数
		try {
			PrintWriter pw = super.getHttpResponse().getWriter();
			// 调用根据对象参数信息，对于信息新增修改时是否存在重复数据
			List<TransactionTimeConfig> list = transactionTimeConfigService
					.findParameterCollection(detail);
			if (list.size() < 1) {// 判断是修改
				boolean flag = transactionTimeConfigService
	...
	public String remove() {
		...
		try {
			PrintWriter pw = super.getHttpResponse().getWriter();
			transactionTimeConfigService.removeParameterCollectionByuuid(uuid);
			pw.flush();
			pw.close();
		} catch (IOException e) {
			e.printStackTrace();
