 ...
 * 
 */
public class TransactionEndConfigAction extends BaseAction {

	private static final long serialVersionUID = 1L;
	private TransactionEndConfigService transactionEndConfigService = (TransactionEndConfigService) super
			.getBean("transactionEndConfigService");
	...
	public String findTransactionTimeList() {
		...
		// 调用按照分页查询信息的方法
		Page page = this.transactionEndConfigService
	 ...
	 */
	public String save() {
		TransactionEndConfig detail = (TransactionEndConfig) BeanUtil
				.getObject(getHttpRequest(), TransactionEndConfig.class);
		// 通过request方法获取页面传递的参数
		try {
			PrintWriter pw = super.getHttpResponse().getWriter();
			// 调用根据对象参数信息，对于信息新增修改时是否存在重复数据
			List<TransactionEndConfig> list = transactionEndConfigService
					.findParameterCollection(detail);
			if (list.size() < 1) {// 判断是修改
				boolean flag = transactionEndConfigService
	...
	public String remove() {
		...
		try {
			PrintWriter pw = super.getHttpResponse().getWriter();
			transactionEndConfigService.removeParameterCollectionByuuid(uuid);
			pw.flush();
			pw.close();
		} catch (IOException e) {
			e.printStackTrace();
