...
(function($) {
		...
		if (_5c.buttons) {
			...
			for (var i = 0; i < _5c.buttons.length; i++) {
				...
				if (btn == "-") {
					...
					$("<a href=\"#\"></a>").addClass("l-btn").css("float", "left").text(btn.text || "").attr("icon", btn.iconCls || "").bind("click", eval(btn.handler ||
		...
		function _cb(ul, _ce, _cf) {
			for (var i = 0; i < _ce.length; i++) {
				...
				$.data(_d1[0], "tree-node", {
					...
					propertys: _d0.propertys	// edit by yaodan
				});
	...
	function _187(_188) {
		...
		if (opts.title && !opts.noheader) {
			...
			if (opts.collapsible) {
				$("<a class=\"panel-tool-collapse\" href=\"#\"></a>").appendTo(tool).bind("click",
				function() {
			...
			if (opts.minimizable) {
				$("<a class=\"panel-tool-min\" href=\"#\"></a>").appendTo(tool).bind("click",
				function() {
					_1aa(_188);
					return false;
				});
			...
			if (opts.maximizable) {
				$("<a class=\"panel-tool-max\" href=\"#\"></a>").appendTo(tool).bind("click",
				function() {
			...
			if (opts.closable) {
				$("<a class=\"panel-tool-close\" href=\"#\"></a>").appendTo(tool).bind("click",
				function() {
					_18b(_188);
					return false;
				});
	...
	$.fn.panel.parseOptions = function(_1c7) {
		...
		return {
			...
			uuid: t.attr("uuid"),				//使用portal添加  吕新亮
			userId: t.attr("userId"),
			portalId: t.attr("portalId")
		};
	};
	$.fn.panel.defaults = {
		uuid: "",								//使用portal添加  吕新亮
			...
			if (typeof opts.toolbar == "string") {
				...
				for (var i = 0; i < opts.toolbar.length; i++) {
					...
					if (p == "-") {
						...
						var tool = $("<a href=\"#\"></a>").appendTo(_1ee);
						tool.css("float", "left");
		...
		if (opts.buttons) {
			if (typeof opts.buttons == "string") {
				...
				for (var i = 0; i < opts.buttons.length; i++) {
					...
					var _1f0 = $("<a href=\"#\"></a>").appendTo(_1ef);
					if (p.handler) {
	...
	function _205(_206, _207, _208) {
		...
		if (_208) {
			...
			for (var _209 in _208) {
				$("<a></a>").attr("href", "#").text(_209).css("margin-left", 10).bind("click", eval(_208[_209])).appendTo(tb).linkbutton();
			}
		}
		win.window({
			title: _206,
	...
	function _25c(_25d) {
		...
		if (opts.tools) {
			if (typeof opts.tools == "string") {
				...
				for (var i = 0; i < opts.tools.length; i++) {
					var tool = $("<a href=\"#;\"></a>").appendTo(_25f);
	...
	function _276(_277, pp, _278) {
		...
		var _27a = $("<a href=\"#\" class=\"tabs-inner\"></a>").appendTo(tab);
		...
		pp.panel($.extend({},
		{
			...
			onOpen: function() {
				...
				if (!tool.children("a." + _2c1).length) {
					var t = $("<a href=\"#\"></a>").addClass(_2c1).appendTo(tool);
					t.bind("click", {
						dir: dir
					},
					function(e) {
	...
	function _750(_751) {
		...
		function _753() {
			...
			$("<a href=\"#\" class=\"datebox-current\"></a>").html(opts.currentText).appendTo(_756);
			$("<a href=\"#\" class=\"datebox-close\"></a>").html(opts.closeText).appendTo(_756);
	...
	function _766(_767) {
		...
		if (!_768.spinner) {
			...
			var ok = $("<a href=\"#\" class=\"datebox-ok\"></a>").html(opts.okText).appendTo(_76b);
			ok.hover(function() {
				$(this).addClass("datebox-button-hover");
			},
			function() {
	...
	function init(_783) {
		var _784 = $("<div class=\"slider\">" + "<div class=\"slider-inner\">" + "<a href=\"#\" class=\"slider-handle\"></a>" + "<span class=\"slider-tip\"></span>" + "</div>" + "<div class=\"slider-rule\"></div>" + "<div class=\"slider-rulelabel\"></div>" + "<div style=\"clear:both\"></div>" + "<input type=\"hidden\" class=\"slider-value\">" + "</div>").insertAfter(_783);
...
(function($) {
		...
		function _cb(ul, _ce, _cf) {
			for (var i = 0; i < _ce.length; i++) {
				...
				$.data(_d1[0], "tree-node", {
					...
					iconCls: _d0.iconCls,
					attributes: _d0.attributes,
					propertys: _d0.propertys	//eidt by yaodan
				});
				$("<span class=\"tree-title\"></span>").html(_d0.text).appendTo(_d1);
				if (_c8.checkbox) {
					if (_c8.onlyLeafCheck) {
	...
	$.fn.tree.parseOptions = function(_15d) {
		...
		return {
			...
			//end
			propertys : t.attr("propertys") //edit by yaodan 保存节点额外的属性， 如机构需要保存机构码, 配置propertys: ['orgCode']	
		};
	};
	$.fn.tree.defaults = {
		url: null,
		...
		loadFilter: function(data, _161) {  //edit by duyong 扩展树组件，使得展示数据可用扁平pid结构数据集
			...
			if (opt.parentField) {
				...
				propertys = opt.propertys;	//edit by yaodan
				var i,
				l,
				treeData = [],
				tmpMap = [];
				...
				for (i = 0, l = data.length; i < l; i++) {
					if (tmpMap[data[i][parentField]] && data[i][idField] != data[i][parentField]) {
						...
						//begin edit by yaodan
						data[i]['propertys'] = new Object();
						for(t = 0, ln = propertys.length; t < ln; t++) {
							data[i]['propertys'][propertys[t]] = data[i][propertys[t]];
						}
						//end
	...
	function _1cf(_1d0, _1d1) {
		...
		if (_1d2.shadow) {
			_1d2.shadow.css({
				left: _1d2.options.left-5,
				top: _1d2.options.top-5
			});
		}
	};
	function _1d3(_1d4) {
			...
			onOpen: function() {
				...
				if (_1d5.shadow) {
					_1d5.shadow.css({
						display: "block",
						zIndex: $.fn.window.defaults.zIndex++,
						left: _1d5.options.left-5,
						top: _1d5.options.top-5,
						width: _1d5.window.outerWidth(),
						height: _1d5.window.outerHeight()
					});
				}
	...
	function _205_self(_206, _207, _208,top,left,bottom,right) {
		var win = $("<div class=\"messager-body\"></div>").appendTo("body");
		if(top!=undefined){
			$(".messager-body").css("top",top);
		}
		if(left!=undefined){
			$(".messager-body").css("left",left);
		}
		if(bottom!=undefined){
			$(".messager-body").css("bottom",bottom);
		}
		if(right!=undefined){
			$(".messager-body").css("right",right);
		}
		win.append(_207);
		if (_208) {
			var tb = $("<div class=\"messager-button\"></div>").appendTo(win);
	...
	$.messager = {
		...
		alertSelf: function(_20b, msg, icon,top,left,bottom,right, fn) {
			var _20c = "<div>" + msg + "</div>";
			switch (icon) {
			case "error":
				_20c = "<div class=\"messager-icon messager-error\"></div>" + _20c;
			...
			var win = _205_self(_20b, _20c, _20d,top,left,bottom,right);
		},
		confirm: function(_20e, msg, fn) {
			var _20f = "<div class=\"messager-icon messager-question\"></div>" + "<div>" + msg + "</div>" + "<div style=\"clear:both;\"/>";
			var _210 = {};
		...
		confirmSelf: function(_20e, msg,fn,top,left,bottom,right) {
			var _20f = "<div class=\"messager-icon messager-question\"></div>" + "<div>" + msg + "</div>" + "<div style=\"clear:both;\"/>";
			var _210 = {};
			_210[$.messager.defaults.ok] = function() {
				win.window("close");
			...
			var win = _205_self(_20e, _20f, _210,top,left,bottom,right);
		},
		prompt: function(_211, msg, fn) {
			var _212 = "<div class=\"messager-icon messager-question\"></div>" + "<div>" + msg + "</div>" + "<br/>" + "<input class=\"messager-input\" type=\"text\"/>" + "<div style=\"clear:both;\"/>";
			var _213 = {};
	...
	function _356(_357) {
		...
		var _35a = box.attr("disabled");
		if (_35a == true || _35a == "true" || _35a == "disabled") {
			return true;
		}
		if (opts.required) {
			if (_358 == "") {
	...
	$.fn.validatebox.defaults = {
		...
		rules: {
			...
			number: {
				...
				message: "Please enter a number of length from {0} to {1}. "
			},
			email: {
				...
				for (var i = 0; i < opts.toolbar.length; i++) {
					...
					if (btn == "-") {
						...
						if(btn.title==undefined){
							...
							tool = $("<a href=\"#\" title=\"" + btn.title + "\"></a>");
						}
						tool[0].onclick = eval(btn.handler ||
						function() {});
						//判断属性配置
						if(btn.menubutton != null){
				...
				for (var j = 0; j < cols.length; j++) {
					...
					if (col.checkbox) {
						...
						if(opts.singleSelect){
							...
							$("<div class=\"datagrid-header-check\"></div>").html("<input title=\"全选/反选\" type=\"checkbox\"/>").appendTo(td);
						}
					} else {
						if (col.field) {
							td.attr("field", col.field);
	...
	function _48d(_494) {
		...
		//单选
		if(opts.singleSelect){
			_496 = tr.find("div.datagrid-cell-check input[type=radio]");
		}else{
			_496 = tr.find("div.datagrid-cell-check input[type=checkbox]");
		}
		$.fn.prop ? _496.prop("checked", false) : _496.attr("checked", false);
		if (opts.idField) {
			for (var _497 = 0; _497 < data.rows.length; _497++) {
	...
	function _44c(_498, _499) {
		...
		if (!tr.hasClass("datagrid-row-selected")) {
			...
			var ck;
			if(opts.singleSelect){
				ck = $("div.datagrid-cell-check input[type=radio]", tr);
			}else{
				ck = $("div.datagrid-cell-check input[type=checkbox]", tr);
			}
			$.fn.prop ? ck.prop("checked", true) : ck.attr("checked", true);
			if (opts.idField) {
				var row = data.rows[_499]; (function() {
	...
	function _4aa(_4ab, _4ac, _4ad) {
		...
		if (!_4ad) {
			...
			tr.find("div.datagrid-editable").each(function() {
				...
				var ed = $.data(this, "datagrid.editor");
				var _4b3 = ed.actions.getValue(ed.target);
				//当可编辑表格中存在弹出树时修改判断对象是否相等的错误问题
				//haoliang
				//将对象转换为JSON进行比较   lxl edit
				var aa = JSON.stringify(row[_4b2]);
				if (JSON.stringify(row[_4b2])!=JSON.stringify(_4b3)) {
					row[_4b2] = _4b3;
					_4b0 = true;
					_4b1[_4b2] = _4b3;
				}
	...
	function _4ea(_4eb, _4ec) {
		...
		//begin grid与查询form表单结合  by tanghui
		if(opts.queryForm){
			//获得表单的所有数据，数据类型为数组
			var formParams = $('#' + opts.queryForm).serializeArray();
			if(formParams && formParams.length > 0){
				for(var i = 0; i < formParams.length; i++){
					//将表单数据添加到opts.queryParams（额外参数）属性中，异步访问数据时会
					var param = formParams[i];
					opts.queryParams[param.name] = param.value;
				}
			}
		}
		//endgrid 与查询form表单结合
	...
	var _4f8 = {
		...
		checkbox: {
			init: function(_509, _50a) {
				var _50b = $("<input type=\"radio\">").appendTo(_509);
				_50b.val(_50a.on);
				_50b.attr("offval", _50a.off);
				return _50b;
			},
	...
	$.fn.datagrid.methods = {
		...
		//begin grid与form结合后 对外提供js接口   by tanghui
		getQueryForm: function(jq){
			return $.data(jq[0], "datagrid").options.queryForm;
		},
		getQueryFormParam: function(jq) {              
			var queryForm = $.data(jq[0], "datagrid").options.queryForm;
			var formParams = $('#' + queryForm).serializeArray();//数据为数组类型, 如：formParams[0].name=username，formParams[0].value=张三
			return formParams;
		},
		//end grid与form结合后 对外提供js接口
		getPanel: function(jq) {
			return $.data(jq[0], "datagrid").panel;
		},
		getPager: function(jq) {
	...
	$.fn.datagrid.parseOptions = function(_55f) {
		...
		$.fn.panel.parseOptions(_55f), {
			...
			sortOrder: t.attr("sortOrder"),
			//begin grid与查询form表单结合  by tanghui
			queryForm: t.attr("queryForm"), 
			//end grid 与查询form表单结合
			showHeader: (t.attr("showHeader") ? t.attr("showHeader") == "true": undefined),
			showFooter: (t.attr("showFooter") ? t.attr("showFooter") == "true": undefined),
			scrollbarSize: (t.attr("scrollbarSize") ? parseInt(t.attr("scrollbarSize")) : undefined),
			loadMsg: (t.attr("loadMsg") != undefined ? t.attr("loadMsg") : undefined),
	...
	//begin 根据字符长度计算前端展示所占用的宽度的像素值   add by tanghui 
	function getFontLength(string){
		var length = 0;
		if(!string) return 120;
		for(var i = 0; i < string.length; i++){
			var chars = string.charAt(i);
			if(/([0-9a-zA-Z])/.test(chars)){
				length += 7;
			}else{
				length += 12;
			}
	...
	function _700(_701, data, _702) {
		...
		var _704 = $(_701).combobox("getValues");
		_703.empty();
		//begin 控制下拉面板的宽度随字符长度而自动变化    by tanghui
		if(data && data.length > 0){
			if(opts.panelWidth){
				...
				var width = (opts.width=='auto' ? 87 : opts.width);
				var maxLength = width;
				for (var i = 0; i < data.length; i++) {
					var v = data[i][opts.valueField];
					var s = data[i][opts.textField];
					...
					item.attr("value", v);
					if (opts.formatter) {
						s = opts.formatter.call(_701, data[i]);
						item.html(s);
					} else {
						item.html(s);
					}
					var valueLength = getFontLength(s);
