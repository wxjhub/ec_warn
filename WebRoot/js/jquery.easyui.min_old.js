...
(function($) {
	...
	$.fn.tree.parseOptions = function(_15d) {
		...
		return {
			...
			idField : t.attr("idField"),
			textField : t.attr("textField"),
			parentField : t.attr("parentField")
			//end
		};
	};
	$.fn.tree.defaults = {
		...
		loadFilter: function(data, _161) {  //edit by duyong 扩展树组件，使得展示数据可用扁平pid结构数据集
			...
			tools: [{
				...
				handler: function() {
					...
					if (pp.panel("options").collapsed) {
						...
						if($(".accordion-header-selected")[0] != undefined) {
							return false;
						} else {
							_239(_22f);
							pp.panel("collapse", _231);
		...
		rules: {
			idcard:{
				validator:function(value, param){
					return /^[\d]{6}((19[\d]{2})|(200[0-8]))((0[1-9])|(1[0-2]))((0[1-9])|([12][\d])|(3[01]))[\d]{3}[0-9xX]$/.test(value);
				},
			...
			mobilePhone:{
					validator:function(value, param){
					return /^0?1[358]\d{9}$/.test(value);
				},
				message:"Please enter a valid mobilePhone."
			},	
			name:{
				validator:function(value, param){
			...
			for (var i = 0; i < _43b.length; i++) {
				...
				for (var j = 0; j < cols.length; j++) {
					...
					if (col.checkbox) {
						...
						//当网格只允许单选时，全选复选框不显示
						if(opts.singleSelect){
							$("<div class=\"datagrid-header-check\"></div>").html("<div style=\"width:13px;border:none;background:none\"></div>").appendTo(td);
						}else{
							$("<div class=\"datagrid-header-check\"></div>").html("<input type=\"checkbox\"/>").appendTo(td);
						}
					} else {
		...
		loading: function(jq) {
			return jq.each(function() {
				...
				if (opts.loadMsg) {
					var _54a = $(this).datagrid("getPanel");
					$("<div class=\"datagrid-mask\" style=\"display:block\"></div>").appendTo(_54a);
					$("<div class=\"datagrid-mask-msg\" style=\"display:block;padding-top:30px\"></div>").html(opts.loadMsg).appendTo(_54a);
					_40f(this);
				}
			});
		},
	...
	$.fn.datebox.defaults, {
		...
		parser: function(s) {
			...
			var d = $.fn.datebox.defaults.parser(s.substring(0,s.lastIndexOf(" ")));
			var tt = s.substring(s.lastIndexOf(" ")+1).split(":");
//			var dt = s.split(" ");
//			var d = $.fn.datebox.defaults.parser(dt[0]);
//			var tt = dt[1].split(":");
			var hour = parseInt(tt[0], 10);
