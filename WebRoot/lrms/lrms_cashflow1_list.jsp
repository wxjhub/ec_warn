	...
	<form name="formtrSearch" id="formtrSearch" method="post" style="padding-left: 20px;">
		...
		<table cellSpacing="0" cellpadding="0" border="0">
			<tr>
				...
				<td height="40px"><input type="text" name="processId" id="selectProcessId" disabled="disabled" class="input_eq" style="width: 100px" value="${param.setting}" /></td>
		</table>
		</form>	
		<div class="gridDiv">
		<r:grid sortable="true" remoteSort="true" title="现金流参数配置" pagination="true" idField="uuid" editable="true"   onLoadSuccess="success(par)"  singleEdit="true"  url="cashParameter_findAllCashParameter.action?settingId=\"+$('#selectProcessId1').val()+\"" striped="true" fit="true" height="100%"  onSelect="testOnRelationSelect(a,b);" id="lrmsTaskRelation" onBeforeLoad="testabc();" rownumbers="false" singleSelect="true">
		...
		<r:toolbar text="<input style='border: 1px #c0c0c0 solid; height: 20px; width:160px;' type='text' id='txt1' name='txt1' /><input type='button' style='height: 17px;' class='inputd' value='浏览' /><input class='input_file' type='file' id='upload1' name='upload1' onchange='txt1.value=this.value'/>"></r:toolbar>
		...
		<r:col field="paramName" title="参数名称"  sortable="true"   width="280"   dictId="lrms.paramName">
     	<r:editorDictSelect required="true" missingMessage="不能为空" panelHeight="230"></r:editorDictSelect>
		</r:col>
		<r:col field="limitTime" title="期限" sortable="true"  width="260" rowspan="2" dictId="lrms.limitTime" >
		<r:editorDictSelect required="true" missingMessage="不能为空"></r:editorDictSelect>
		</r:col>
		<r:col field="expiredChurnrate" title="到期流失率" sortable="true"  width="276" rowspan="2">
		</r:col>
		<r:col field="unexpiredChurnrate" title="未到期流失率" sortable="true"  width="260" rowspan="2">
		</r:col>
		<r:pagination id="pag" afterPageText="页" beforePageText="第" pageNO="2" total="99" pageSize="10" displayMsg=" {from}/{to}页 共 {total}条"></r:pagination>
</r:grid>
	...
	if(changes.length>1){
		...
		if(uuid!=""){
				$.ajax({
					...
					type: 'post',
					dataType : "Json",
					data:{uuid:uuid,paramName:paramName,limitTime:limitTime,settingId:settingId,amount:amount,expiredChurnrate:expiredChurnrate,unexpiredChurnrate:unexpiredChurnrate},
					...
					success:function(data){
						...
						//	 batchId=batchId1;
							...
							$.messager.alert('系统提示','不可以将其他现金流流失率，未到期流失率设置为百分数','warning');
							lrmsTaskRelation.reload();
							$('#editRowProcess1').removeAttr("disabled");
							$('#addRowProcess1').removeAttr("disabled");															
						}
