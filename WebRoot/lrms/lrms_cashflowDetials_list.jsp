...
function btnDel(){
	...
	if(btn){
		$.ajax({
			...
			type: 'post',
			dataType : "Json",
			url: "cashParameterDetails_remove.action?uuids="+uuids,
...
function intrExcel(){
	...
	if(updateUpdata!=null&&updateUpdata!=""){
		...
		$.ajaxFileUpload({  
			url: "lrmsinputExcel_inputExcelCashParamterDetails.action?uuid="+abc+"&modelName="+"CashParamterDetails",
			secureuri:false,  
...
function getTemplate(){
	window.location.href="${pageContext.request.contextPath}/lrmsdownloadTemplate.action?modelName="+"CashParamterDetails";
}
...
function getTaskRelationByData(){
		...
		var abc=$('#selectProcessId1').val();
		window.location.href="${pageContext.request.contextPath}/lrmsexportExcel.action?uuid="+abc+"&modelName="+"CashParamterDetails";
...
<body>
	<form name="formtrSearch" id="formtrSearch" method="post" style="padding-left: 20px;">
			...
			<tr>
				...
				<td height="40px">
					...
					<select name="paraName"   id="paraName" >
					    ...
					    <rmp:option dictId="lrms.zsl.paramName"></rmp:option>    
					</select>
				</td>
				<td>
		...
		<div class="gridDiv">
		<r:grid sortable="true" remoteSort="true" title="指标参数配置" pagination="true" idField="uuid" editable="true" onLoadSuccess="success(par)"   singleEdit="true"  url="cashParameterDetails_findAllCashParameter.action" striped="true" fit="true" height="100%"  onSelect="testOnRelationSelect(a,b);" id="lrmsTaskRelation" onBeforeLoad="testabc();" rownumbers="false" singleSelect="true">
		...
		<r:toolbar text="<span style='position:relative;top:-3px;'><input style='border: 1px #c0c0c0 solid; height: 18px; width:240px;' type='text' id='txt1' name='txt1' /><input type='button' style='height: 20px;cur' class='inputd' value='浏览' /><input class='input_file' type='file' id='upload1' name='upload1' onchange='txt1.value=this.value'/></span>"></r:toolbar>
		...
		<r:col field="paramName" title="参数名称"  sortable="true"   width="357"   dictId="lrms.zsl.paramName">
     	<r:editorDictSelect required="true" missingMessage="不能为空" panelHeight="400"></r:editorDictSelect>
		</r:col>
		<r:col field="lessConvrate" title="1以下年折算率" sortable="true"  width="200" rowspan="2">
		<r:editorText required="true" missingMessage="必须为百分数"></r:editorText>
		</r:col>
		<r:col field="moreConvrate" title="1年或者无到期日折算率" sortable="true"  width="200" rowspan="2">
		<r:editorText required="true" missingMessage="必须为百分数"></r:editorText>
		</r:col>
	...
	if(changes.length>1){
		...
		if(uuid!=""){
				$.ajax({
					...
					type: 'post',
					dataType : "Json",
					data:{uuid:uuid,paramName:paramName,lessConvrate:lessConvrate,moreConvrate:moreConvrate},
					url: "cashParameterDetails_save.action",
					success:function(data){
						var result=eval("(" + data + ")");
						if(result==1){
