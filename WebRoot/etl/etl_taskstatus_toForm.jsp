...
<script type="text/javascript">
		...
		if (flag){
		    document.forms['taskFormSave'].action="${pageContext.request.contextPath}/taskStatus_save.action";
        ...
        $(function(){
    		...
    		$("#reset").click(function(){
    			if('${taskStatus.taskId}'){
    				 $('#batchId').val('${taskStatus.batchId}');
    				 $('#taskId').val('${taskStatus.taskId}');
                     $('#processId').val('${taskStatus.processId}');  
                     $('#runFlag').combobox('setValue','${taskStatus.runFlag}');
                     $('#runTime').val('${taskStatus.runTime}');
                     $('#startTime').val('${taskStatus.startTime}');
                     $('#endTime').val('${taskStatus.endTime}');
              	}
		...
		function goBack(){
			window.location.href = "${pageContext.request.contextPath}/process_toStatusConfig.action?batchId="+$("#batchId").val()+"&processId="+$("#processId").val();
		}
	...
	<TR>
		...
		<table>
			<TR>
				...
				<input type="hidden" id="uuid" name="uuid" value="${taskStatus.uuid}"></input>
				<div id="myTab1_Content0">
				<table cellSpacing="0" cellpadding="0" border="0">
					<tr>
						<td width="80px" height="40px">任务编号：</td>
						<td height="40px"><input type="text" onBlur="checkunique()"
							class="easyui-validatebox input_eq" style="width: 150px" readonly="readonly"
							name="taskId" id="taskId" value="${taskStatus.taskId}" />		
						</td>
						<td width="150px" height="40px"></td>
						<td width="80px" height="40px">任务组编号：</td>
						<td height="40px"><input type="text" onBlur="checkunique()"
							class="easyui-validatebox input_eq" style="width: 150px" readonly="readonly"
							name="processId" id="processId" value="${taskStatus.processId}" />		
						</td>
					...
					<tr>
						...
						<td height="40px"><input type="text" onBlur="checkunique()"
							class="easyui-validatebox input_eq" style="width: 150px" readonly="readonly"
							name="batchId" id="batchId" value="${taskStatus.batchId}" />		
						</td>
                        <td width="150px" height="40px"></td>
						<td width="80px" height="40px">运行状态：</td>
						<td height="40px">
						<select id="runFlag" required="true"  missingMessage="请选择运行状态"
						name="runFlag" class="easyui-combobox"  style="width: 150px;">
			                 <rmp:option dictId="etl.task.runflag"  currentValue="${taskStatus.runFlag}"></rmp:option>
						</select></td>
					</tr>
					...
					<tr>
						...
						<input type="text"
							...
							id="startTime" value="<fmt:formatDate value='${taskStatus.startTime}' pattern='yyyy-MM-dd HH:mm:ss'/>" />
						...
						<td height="40px"><input type="text"
							...
							id="endTime" value="<fmt:formatDate value='${taskStatus.endTime}' pattern='yyyy-MM-dd HH:mm:ss'/>" /></td>
					...
					<tr>
						...
						<td height="40px"><input type="text"
							...
							id="runTime" value="<fmt:formatDate value='${taskStatus.runTime}' pattern='yyyy-MM-dd HH:mm:ss'/>" /></td>
