...
<body>
	<%-- <form name="formSearch" id="formSearch" method="post" style="padding: 20px 0 20px 30px;">
		...
		rownumbers="false" singleSelect="false" editable="true" url="sqlBlockConfig_findList.action" 
		striped="true" fit="true" height="100%"  title="sql阻塞监控参数配置" id="lrmsMoni">
			...
			<r:col  field="sqlTimeValue" title="sql时长阀值" editable="true" sortable="true" width="150">
			</r:col>
...
function save(){
			...
			if(vali){
				...
				if(selected.length>1){
					...
					var sqlTimeValue = selected[0].sqlTimeValue;
					...
					var col3 = selected[0].col3;
							...
							$.ajax({
								...
								type: 'post',
								dataType : "Json",
								data:{uuid:uuid,sqlTimeValue:sqlTimeValue,
									warningLevl:warningLevl,remark:remark,col1:col1,col2:col2,col3:col3},
								url: "sqlBlockConfig_save.action",
								error:function(data){
	...
	function remove(){
		...
		if(selected.length== 0) {
			...
			$.ajax({
				...
				type: 'post',
				dataType : "Json",
				url: "${pageContext.request.contextPath}/sqlBlockConfig_remove.action?uuid="+uuids,
				success:function(data){
