...
<body>
	<form name="formSearch" id="formSearch" method="post" style="padding: 20px 0 20px 30px;">
		...
		rownumbers="false" singleSelect="false" editable="true" url="dayendConfig_findList.action" 
		striped="true" fit="true" height="100%"  title="日终监控参数配置" id="lrmsMoni">
			...
			<r:col  field="dayendName" title="日终步骤名称" editable="true" sortable="true" width="150">
			</r:col>
			<r:col  field="dayendNo" title="日终步骤号" editable="true" sortable="true" width="150">
			</r:col>
			<r:col field="dayendDate" title="日切时间" editable="true" sortable="true" width="150" >
				<r:editorDate format="yyyy-MM-dd" />
			</r:col>
			<r:col  field="dayendMaxTime" title="日终最晚发起时间" editable="true" sortable="true" width="150">
			    <r:editorText></r:editorText>
			</r:col>
			<r:col  field="dayendEndTime" title="日终最晚结束时间"  sortable="true" width="100" editable="true">
			    <r:editorText></r:editorText>
			</r:col>
			<r:col  field="dayendState" title="日终步骤状态" sortable="true" width="100" >
			<r:editorText></r:editorText>
			</r:col>
...
function save(){
			...
			if(vali){
				...
				if(selected.length>1){
					...
					var dayendName = selected[0].dayendName;
					var dayendNo = selected[0].dayendNo;
					var dayendDate = selected[0].dayendDate;
					var dayendMaxTime = selected[0].dayendMaxTime;
					var dayendEndTime = selected[0].dayendEndTime;
					var warningLevl = selected[0].warningLevl;
					var dayendState = selected[0].dayendState;
					...
					var col3 = selected[0].col3;
							...
							$.ajax({
								...
								type: 'post',
								dataType : "Json",
								data:{uuid:uuid,dayendName:dayendName,dayendNo:dayendNo,dayendDate:dayendDate,
									dayendMaxTime:dayendMaxTime,dayendEndTime:dayendEndTime,
									warningLevl:warningLevl,dayendState:dayendState,col1:col1,col2:col2,col3:col3},
								url: "dayendConfig_save.action",
								error:function(data){
	...
	function remove(){
		...
		if(selected.length== 0) {
			...
			$.ajax({
				...
				type: 'post',
				dataType : "Json",
				url: "${pageContext.request.contextPath}/dayendConfig_remove.action?uuid="+uuids,
				success:function(data){
