...
function findService(){
	...
	for(var i=0; i<selected.length; i++){
	    uuids.push(selected[i].netIP);
	    ports.push(selected[i].netPort);
	}
...
function save(){
			...
			if(vali){
				...
				if(selected.length>1){
					...
					var hostPort = selected[0].hostPort;
					...
					/* if(!re.test(pointIP)){  
						$.messager.alert('系统提示','<span style="color:red">'+"节点IP地址格式错误"+'</span>','warning');
							...
							$.ajax({
								...
								type: 'post',
								dataType : "Json",
								data:{uuid:uuid,hostName:hostName,hostIp:hostIp,hostPort:hostPort,collectTime:collectTime,warningLevl:warningLevl,col1:col1,col2:col2,col3:col3},
								url: "localNetWorkConfig_save.action",
								error:function(data){
	...
	function remove(){
		...
		if(selected.length== 0) {
			...
			$.ajax({
				...
				type: 'post',
				dataType : "Json",
				url: "${pageContext.request.contextPath}/localNetWorkConfig_remove.action?uuid="+uuids,
				success:function(data){
...
<body>
	<form name="formSearch" id="formSearch" method="post" style="padding: 20px 0 20px 30px;">
		...
		rownumbers="false" singleSelect="false" editable="true" url="localNetWorkConfig_findNetWorkList.action" 
		striped="true" fit="true" height="100%"  title="本端网络监控参数配置" id="lrmsMoni" >
			...
			<r:col  field="hostPort" title="主机端口" sortable="true" width="150" editable="true">
			</r:col>
