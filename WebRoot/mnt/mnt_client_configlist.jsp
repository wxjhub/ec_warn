...
<body>
	<form name="formSearch" id="formSearch" method="post" style="padding: 20px 0 20px 30px;">
		...
		rownumbers="false" singleSelect="false" editable="true" url="clientconfig_findClientConfigList.action" 
		striped="true" fit="true" height="100%"  title="系统基本配置" id="lrmsMoni">
			...
			<r:col  field="subsystemName" title="子系统名称" sortable="true" width="150">
			<r:editorText missingMessage="不能为空" required="true"></r:editorText>
			</r:col>
			<r:col  field="nodeName" title="节点名称" sortable="true" width="100">
			<r:editorText missingMessage="不能为空" required="true"></r:editorText>
			</r:col>
			<r:col  field="ipAddress" title="IP地址" sortable="true" width="300">
			<r:editorText missingMessage="不能为空" required="true"></r:editorText>
			</r:col>
			<r:col  field="informationCenter" title="主机存放信息中心" sortable="true" width="300">
			<r:editorText missingMessage="不能为空" required="true"></r:editorText>
			</r:col>
			<r:col  field="remarks" title="描述" sortable="true" width="300">
			<r:editorText missingMessage="不能为空" required="true"></r:editorText>
			</r:col>
...
function save(){
			...
			if(vali){
				...
				if(selected.length>1){
					...
					var subsystemName = selected[0].subsystemName;
					var nodeName = selected[0].nodeName;
					var ipAddress = selected[0].ipAddress;
					var informationCenter = selected[0].informationCenter;
					var remarks = selected[0].remarks;
							$.ajax({
								cache:false,
								type: 'post',
								dataType : "Json",
								data:{uuid:uuid,systemName:systemName,subsystemName:subsystemName,nodeName:nodeName,ipAddress:ipAddress,informationCenter:informationCenter,remarks:remarks},
								url: "clientconfig_save.action",
								error:function(data){
								...
								success:function(data){
									if(data=="1"){
										...
										$.messager.alert('系统提示','子系统和ip地址已存在，数据保存失败','warning');
									 	lrmsMoni.reload();
									 	$('#editRow').removeAttr("disabled");
										$('#addRow').removeAttr("disabled");
										lrmsMoni_index = null;
	...
	function remove(){
		...
		if(selected.length== 0) {
			...
			$.ajax({
				...
				type: 'post',
				dataType : "Json",
				url: "${pageContext.request.contextPath}/clientconfig_remove.action?uuid="+uuids,
				success:function(data){
